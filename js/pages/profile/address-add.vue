<template>
    <div class="address-add-page">
        <head-top head-title="新增地址" :goback='true'></head-top>
        <section class="adddetail">
            <form action="" v-on:submit.prevent>
                <section class="ui-padding-block">
                    ...
                    <router-link to='/profile/userInfo/address/add/search' class="add-detail">
                        <div class="input-new">
                            <input type="text" placeholder="小区/写字楼/学校等" readonly="readonly" :value="searchAddress" />
                        </div>
                    </router-link>
                    <div class="input-new">
...
import AlertTip from 'src/components/alert-tip'
...
export default {
    data() {
        ...
        computed: {
            ...mapState([
                'userInfo', 'searchAddress', 'addresses', 'newAddress', 'geohash'
            ]),
        ...
        methods: {
            ...
            //保存地址
            async submitThing() {
                let res = await postAddAddress(this.userInfo.user_id, this.mesthree, this.searchAddress, this.geohash, this.message, this.telenum, this.standbytelenum, 0, 1, '公司', 4);
                if (res.message) {
                    this.showAlert = true;
                    ...
                    this.CONFIRM_ADDRESS({
                        ...
                        address_detail: this.searchAddress,
                        geohash: 'wtw37r7cxep4',
                        phone: this.telenum,
                        phone_bk: this.standbytelenum,
                        poi: this.searchAddress,
                        poi_type: 0,
                    });
                    this.$router.go(-1);
                }
