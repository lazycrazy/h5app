<template>
    <div class="remark-page">
        <section v-if="!showLoading">
            <head-top head-title="订单备注" :goback='true'></head-top>
            <section class="quick_remark" v-if="remarkList.remarks.length">
                ...
                <ul class="remark_arr_list_ul">
                    <li class="remark_arr_list_li" v-for="(item,index) in remarkList.remarks" :key="index">
                        <span :class="{first: remarkIndex == 0, last: remarkIndex == item.length - 1, choosed: selectedRemarkText&& selectedRemarkText[index]&&(selectedRemarkText[index][0] == remarkIndex)}" class="remark_item_li" v-for="(remarkItem, remarkIndex) in item" :key="remarkIndex" @click="chooseRemark(index, remarkIndex, remarkItem)">{{remarkItem}}</span>
                    </li>
                </ul>
            </section>
...
export default {
    ...
    methods: {
        ...
        confirmRemark() {
            this.CONFIRM_REMARK({
                remarkText: this.selectedRemarkText,
